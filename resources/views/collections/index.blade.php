                            ...
                            <tbody>
                                @foreach($collections as $index => $collection)
                                    <tr>
                                        ...
                                        <td>{{ $collection->loan_payment_amount ? '₹' . amountToPoints($collection->loan_payment_amount, 2) : '-' }}
                                        </td>
                                        <td>{{ $collection->loan_payment_date ? $collection->loan_payment_date->format('d/m/Y') : '-' }}
                                        </td>
                                        <td>{{ $collection->ac_transfer_amount ? '₹' . amountToPoints($collection->ac_transfer_amount, 2) : '-' }}
                                        </td>
                                        <td>{{ $collection->ac_transfer_date ? $collection->ac_transfer_date->format('d/m/Y') : '-' }}
                                        </td>
                                        <td>{{ $collection->other_mode_amount ? '₹' . amountToPoints($collection->other_mode_amount, 2) : '-' }}
                                        </td>
                                        <td>{{ $collection->other_mode_date ? $collection->other_mode_date->format('d/m/Y') : '-' }}
                                        </td>
