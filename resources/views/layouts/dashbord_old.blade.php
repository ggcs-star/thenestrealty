@if(auth()->check() || auth('employee')->check())
    @php
        ...
        if (auth('employee')->check()) {
            ...
            $totalPartners = \App\Models\ChannelPartner::where('employee_id', $employeeId)->count();
            $totalProject = \App\Models\Project::where('assigned_employee', $employeeId)->count();
            $totalBooking = \App\Models\Booking::where('employee_id', $employeeId)->count();
           $totalCollection = \App\Models\Collection::where('employee_id', $employeeId)->count();

            $totalCommission = \App\Models\Commission::whereHas('booking', function ($q) use ($employeeId) {
                $q->where('employee_id', $employeeId);
            })->count();

            $totalEmployees = 1;

            $allFollowUps = \App\Models\Collection::with('booking')
            ...
            $latestBooking = \App\Models\Booking::where('employee_id', $employeeId)
            ...
            $latestBooking = \App\Models\Booking::orderBy('booking_date')->get();
        }
                        ...
                        <div class="flex items-center justify-between p-2 bg-gray-50 rounded">
                            <div>
                                ...
                                <p class="text-sm text-gray-500">
                                    Amount: ₹{{ amountToPoints($item->amount) }}<br>
                ...
                <div class="space-y-3">
                    @forelse ($todayBooking as $booking)
                        <div class="flex items-center justify-between p-2 bg-gray-50 rounded">
                            ...
                            <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-sm">
                                ₹{{ amountToPoints($booking->total_amount ?? 0, 2) }}
                            </span>
                        </div>
                    @empty
